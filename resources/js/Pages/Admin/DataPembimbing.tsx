...
interface Pembimbing { id: number; nip: string; name: string; phone: string; department: string; dudis: DudiItem[]; account_status?: string; username?: string; }
...
/* ==================== MULTI-SELECT DUDI DROPDOWN ==================== */
function MultiSelectDudiDropdown({ dudiList, value, onChange }: { dudiList: DudiItem[]; value: number[]; onChange: (v: number[]) => void }) {
    ...
    const selectedItems = dudiList.filter(d => value.includes(d.id));
    ...
    return (
        <div className="relative" ref={ref}>
            <button type="button" onClick={() => { setOpen(!open); setFilter(''); }} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 text-xs text-left flex items-center justify-between bg-white min-h-[42px]">
                <div className="flex flex-wrap gap-1 flex-1 mr-2">
                    {selectedItems.length === 0 ? (
                        <span className="text-slate-500">— Belum Ditentukan —</span>
                    ) : (
                        selectedItems.map(d => (
                            <span key={d.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[11px] font-semibold">
                                {d.name}
                                <button
                                    type="button"
                ...
                <span className="material-symbols-outlined text-sm text-slate-400 shrink-0">{open ? 'expand_less' : 'expand_more'}</span>
            </button>
            {open && (
                    ...
                    <div className="max-h-56 overflow-y-auto">
                        ...
                        {value.length > 0 && (
                            <button
                                type="button"
                                onClick={() => { onChange([]); }}
                                className="w-full text-left px-3 py-2 text-xs hover:bg-red-50 transition-colors flex items-center gap-2 border-b border-slate-100 text-red-500"
                            >
                                <span className="material-symbols-outlined text-sm">remove_selection</span>
                                Hapus Semua Pilihan
                            </button>
                        )}
                        {filtered.length === 0 && (
                        ...
                        {filtered.map(d => (
                            <button
                                ...
                                type="button"
                                onClick={() => toggle(d.id)}
                                className={`w-full text-left px-3 py-2.5 text-xs hover:bg-primary/5 transition-colors flex items-center justify-between border-t border-slate-50 ${value.includes(d.id) ? 'bg-primary/10 text-primary font-semibold' : 'text-slate-700'}`}
                            >
                                <span className="truncate pr-2">{d.name}</span>
                                {value.includes(d.id) && <span className="material-symbols-outlined text-sm text-primary shrink-0">check</span>}
                            </button>
                        ))}
                    </div>
                </div>
...
export default function DataPembimbing({ pembimbings, dudiList, filters }: Props) {
    ...
    const handleAdd = () => { setShowAddDropdown(false); setEditing({ nip: '', name: '', phone: '', department: '', dudi_ids: [] }); setIsAdding(true); };
    const handleEdit = (p: Pembimbing) => { setIsAdding(false); setEditing({ ...p, dudi_ids: (p.dudis || []).map(d => d.id) }); };
    const handleSave = () => {
        const payload = { ...editing, dudi_ids: editing.dudi_ids || [] };
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[900px]">
                        <thead><tr className="bg-slate-50 text-slate-500 text-[10px] font-bold uppercase tracking-wider">
                            ...
                            <th className="px-6 py-4">NIP</th><th className="px-6 py-4">Nama Pembimbing</th><th className="px-6 py-4">Jurusan</th><th className="px-6 py-4">Tempat PKL (DUDI)</th><th className="px-6 py-4">Info Login</th><th className="px-6 py-4 text-center">Status Akun</th><th className="px-6 py-4 text-right">Aksi</th>
                        </tr></thead>
                        <tbody className="divide-y divide-slate-100">
                            {pembimbings.data.map((p) => (
                                <tr key={p.id} className={`hover:bg-slate-50/50 transition-colors ${selectedIds.includes(p.id) ? 'bg-primary/5' : ''}`}>
                                    ...
                                    <td className="px-6 py-4 text-xs text-slate-600">
                                        {p.dudis && p.dudis.length > 0 ? (
                                            <div className="flex flex-wrap gap-1">
                                                {p.dudis.map(d => (
                                                    <span key={d.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[11px] font-medium">
                            ...
                            {pembimbings.data.length === 0 && <tr><td colSpan={8} className="px-6 py-8 text-center text-slate-500 text-sm">Belum ada data pembimbing.</td></tr>}
                        </tbody>
                    </table>
                </div>
                    ...
                    <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4">
                        ...
                        <div className="p-6 space-y-4">
                            <div><label htmlFor="nip" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">NIP</label>
                                <input id="nip" title="NIP Pembimbing" type="text" value={editing.nip} onChange={(e) => setEditing({ ...editing, nip: e.target.value })} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm" placeholder="Masukkan NIP" /></div>
                            <div><label htmlFor="name" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nama Lengkap</label>
                                <input id="name" title="Nama Lengkap Pembimbing" type="text" value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm" placeholder="Masukkan nama" /></div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label htmlFor="department" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Jurusan</label>
                                    <input id="department" title="Jurusan Pembimbing" type="text" value={editing.department} onChange={(e) => setEditing({ ...editing, department: e.target.value })} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm" placeholder="Jurusan" /></div>
                                <div><label htmlFor="phone" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">No. Telepon</label>
                                    <input id="phone" title="No Telepon Pembimbing" type="text" value={editing.phone} onChange={(e) => setEditing({ ...editing, phone: e.target.value })} className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm" placeholder="08xxxxxxxxxx" /></div>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                                    Tempat PKL (DUDI)
                                    {editing.dudi_ids?.length > 0 && (
                                        <span className="ml-2 normal-case font-normal text-primary bg-primary/10 px-2 py-0.5 rounded-full text-[10px]">
                                            {editing.dudi_ids.length} dipilih
                                        </span>
                                    )}
                                </label>
                                <MultiSelectDudiDropdown
                                    dudiList={dudiList}
                                    value={editing.dudi_ids || []}
                                    onChange={(v) => setEditing({ ...editing, dudi_ids: v })}
                                />
                                <p className="mt-1.5 text-[11px] text-slate-400">Pembimbing bisa lebih dari 1 DUDI</p>
                            </div>
                        </div>
